...
#
# NOTE (verify at test-install): named-profile auth scoping (`--profile pablo`)
# is the assumed shape; confirm on first run.
set -euo pipefail
. "$(dirname "$0")/lib.sh"
...
read -r -p "Authenticate the Nous Account now? [y/N] " a; [ "$a" = y ] && \
  sshi "hermes --profile '$PROFILE' auth add nous --type oauth --no-browser --manual-paste"
...
if [ "$a" = y ]; then
  ...
  sshi "hermes --profile '$PROFILE' mcp login google_personal"

  echo; log "WORK account — use an incognito window, sign in with the WORK address:"
  sshi "hermes --profile '$PROFILE' mcp login google_work"
...
echo; log "verify: ssh root@$ip 'hermes --profile $PROFILE mcp list'  → google_work = 11 selected"
